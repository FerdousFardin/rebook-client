...
export default function CategoryProduct() {
  ...
  const {
    ...
    queryFn: () =>
      fetch(`https://rebook-server.vercel.app/products?categoryId=${id}`, {
        headers: {
        ...
        <div className="xl:container m-auto text-gray-600 md:px-12 xl:px-6">
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            {products.map((product) => (
              <div
                key={product._id}
                className="group sm:p-8 rounded-3xl bg-white border border-gray-100 dark:shadow-none dark:border-gray-700 dark:bg-gray-800 bg-opacity-50 shadow-2xl shadow-gray-600/10"
              >
                <div className="lg:relative lg:overflow-hidden rounded-xl">
                  <img
                    src={product.img}
                    alt={product.name}
                    loading="lazy"
                    className="h-96 p-6 lg:p-8 w-full object-cover object-center transition duration-500 group-hover:scale-105"
                  />
                </div>
                <div className="mt-6 relative">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {product.isReported ? (
                      <span className="lg:px-5 py-2 bg-green-300 h-10 rounded-lg text-white">
                        Reported
                      </span>
                    ) : (
                      <button
